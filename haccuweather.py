...
def output():
    ...
    tab = '&nbsp;&nbsp;&nbsp;&nbsp;'
    with open('first_half.txt', 'r') as myfile:
            my_output += myfile.read()

    for i in range( len(events) ):
        day = today + datetime.timedelta(days=i)
        my_output += ('Events on ' + str(day) + ':<br>')
        for n in range( len(events[i] ) ):
            my_output += ( '<b>' + tab + events[i][n][1] + ' at ' + events[i][n][0] + ':</b><br>' )
            my_output += ( tab + tab + str(my_temps[i][n]) + 'F ' + getApparelTemp(my_temps[i][n]) + '<br>')
            if int(events[i][n][2]) >= 21 or int(events[i][n][2]) <= 7:
                my_output += ( tab + tab + getApparelWeather(my_types[i][n][1]) + '<br>')
            else:
                my_output += ( tab + tab + getApparelWeather(my_types[i][n][0]) + '<br>')
        my_output += '<br>'
    with open('second_half.txt', 'r') as myfile:
        my_output += myfile.read()
    return my_output
